...
class Visualizer:
    ...
    def create_visualization(self, viz_type: str, df: pd.DataFrame, 
                            ...
                            z: Optional[str] = None, # Accept z here
        ...
        Args:
            ...
            z: Column name for the z-axis (specifically for heatmaps).
        ...
        required_cols = [col for col in [x, y, color, z] if col is not None]
        ...
        fig = None
        try:
            print(f"Visualizer attempting to create {viz_type} chart with columns: x={x}, y={y}, color={color}, z={z}")
            
            if viz_type == 'heatmap':
                if x and y and z:
                    try:
                        pivot_df = df.pivot(index=y, columns=x, values=z)
                        # --- FIX HERE: Fill empty spots (NaN) with 0 ---
                        ...
                        return None
                else: # Fallback to correlation heatmap if x, y, z are not provided
                    numeric_df = df.select_dtypes(include=['number'])
                    if not numeric_df.empty:
                        corr_df = numeric_df.corr()
                        fig = px.imshow(corr_df, title=title or "Correlation Heatmap", **kwargs)
                    else:
                        print("Error: No numeric columns found for correlation heatmap.")
                        return None
            elif viz_type == 'bar':
    ...
    def recommend_visualization(self, columns: List[str]) -> Dict[str, Any]:
        ...
        if len(columns) == 1:
            ...
            if num_col1 and num_col2:
                ...
                return {'type': 'bar', 'x': col1, 'color': col2}
